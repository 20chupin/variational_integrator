...
def quintuple_pendulum(time: float = 5, time_step: float = 0.05):
    """ """
    biorbd_casadi_model = biorbd_casadi.Model(Models.QUINTUPLE_PENDULUM.value)

    alpha = 1.1
    q_t0 = np.array([0, 0, np.pi / 2, alpha, np.pi - alpha, alpha, - alpha])
    ...
    constraint1 = (biorbd_casadi_model.markers(q_sym)[10].to_mx() - biorbd_casadi_model.markers(q_t0)[10].to_mx())[1:]
    # The origin of the third pendulum is constrained to the tip of the second pendulum
    constraint2 = (biorbd_casadi_model.markers(q_sym)[13].to_mx() - biorbd_casadi_model.markers(q_t0)[13].to_mx())[1:]
    ...
    b = bioviz.Viz(Models.QUINTUPLE_PENDULUM.value)
